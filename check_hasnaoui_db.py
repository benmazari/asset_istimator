...
with conn.cursor() as cur:
    cur.execute("""
        ...
        LEFT JOIN res_company rc ON rc.id = aaa.company_id
...
print(f"Total assets found for TEKNACHEM in 2025: {len(rows)}")
print(f"{'Asset ID':<10} | {'Nom Immo':<35} | {'Cat ID':<6} | {'Nom Catégorie':<30} | {'Date Comptab'}")
print("-" * 105)
for r in rows:
    aid, aname, cid, cname, pdate, cdate = r
    print(f"{aid:<10} | {aname[:33]:<35} | {cid:<6} | {cname[:28]:<30} | {str(cdate)}")
